...
from ..ui import detail, error, status, success
...
def stage(
    ...
    if not files:
        status("No posts to stage.")
        return
    for path in files:
        dest, n = staging.stage_one(path, repo, tag, to_posts, force)
        if dest is None:
            detail(f"  ↷ exists, skip: {os.path.basename(path)}")
        else:
...
def promote(
    ...
    if dest is None:
        error(f"✗ No draft at _drafts/{slug}.md")
        raise typer.Exit(1)
    success(f"✓ promoted: _drafts/{slug}.md → {os.path.relpath(dest, repo)}")
...
def run_menu(repo: str) -> None:
    ...
    if not drafts:
        status("No drafts to promote.")
        return
    status("Drafts in _drafts/:\n")
    for i, d in enumerate(drafts, 1):
        with open(d, encoding="utf-8") as f:
            date = staging.front_matter_date(f.read()) or "????-??-??"
        status(f"  {i:2}. [{date}]  {os.path.basename(d)[:-3]}")
    try:
    ...
    for i in idxs:
        ...
        dest = staging.promote_one(slug, repo)
        if dest is None:
            error(f"  ✗ {slug} — draft vanished, skipped")
            continue
        success(f"  ✓ {slug} → {os.path.relpath(dest, repo)}")
