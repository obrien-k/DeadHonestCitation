...
class FrontMatter(TypedDict, total=False):
    """Fields liftable from a passthrough file's front-matter block."""
...
def parse_simple_front_matter(text: str) -> tuple[FrontMatter, str]:
    """Split a leading YAML front-matter block from a Markdown file.

    Lifts title/date/description/tags without a YAML dependency.

    Returns:
        (meta, body) — the lifted fields (empty when there is no block) and
